...
export default function RentReceiptGenerator() {
  ...
  const validate = (): boolean => {
    ...
    if (Number(data.rentAmount) <= 0) e.rentAmount = 'Enter an amount greater than zero';
    if (!data.address.trim()) e.address = 'Required';
    if (data.landlordPan && !/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(data.landlordPan)) e.landlordPan = 'Enter a valid PAN';
    setErrors(e);
  ...
  const downloadPdf = () => {
    ...
    doc.setProperties({
      title: `Rent Receipt ${data.month} ${data.year}`,
      subject: 'Rent receipt generated by JaldiDocs',
      creator: 'JaldiDocs',
      author: data.landlordName || 'JaldiDocs user',
    });
    const w = 148, m = 12;
    let y = m;
    ...
    row('Rent Amount', `Rs. ${Number(data.rentAmount).toLocaleString('en-IN')}/-`);
    ...
    doc.save(`rent-receipt-${safeFilePart(`${data.month}-${data.year}-${data.tenantName}`, 'draft')}.pdf`);
  };
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card p-6 space-y-4">
          ...
          <div>
            ...
            {errors.landlordPan && <p className="error-text">{errors.landlordPan}</p>}
